                            ...
                            if res_partner:
                                ...
                                if order_line_list:
                                    ...
                                    for sl in ele.get('shipping_lines'):
                                        ...
                                        if shipping:
                                            shipping_product_id = self.env['product.product'].sudo().search([('id', '=', shipping.product_id.id)],limit=1)
    ...
    def woo_order_create(self, data):
        ...
        if not sale_order:
            ...
            if res_partner:
                ...
                for line in ele.get('coupon_lines'):
                    woo_coupon_id = line['meta_data'][0]['value']['id']
                    coupon = self.env['loyalty.program'].sudo().search([('woo_id', '=', woo_coupon_id)])
                    if coupon:
                        if coupon.discount_specific_product_ids:
                            for coupon_product in coupon.discount_specific_product_ids:
                                vals = {
                                ...
                                coupon_line = self.env['sale.order.line'].sudo().create(vals)
