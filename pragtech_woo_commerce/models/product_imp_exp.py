                ...
                if data.content:
                    ...
                    if parsed_data:
                        for ele in parsed_data:
                            ...
                            dict_p['description_sale'] = ele.get('description').replace('<p>', '').replace('</p>', '') if ele.get('description') else ''
                            ...
                            dict_p['purchase_ok'] = True if ele.get('purchaseable') and ele.get('purchaseable') == True else False
                            ...
                            dict_p['woo_product_length'] = float(ele.get('dimensions').get('length'))  if ele.get('dimensions') and ele.get(
                                'dimensions').get('length') else 0.00
                            dict_p['woo_product_width'] = float(ele.get('dimensions').get('width')) if ele.get('dimensions') and ele.get(
                                'dimensions').get('width') else 0.00
                            dict_p['woo_product_height'] = float(ele.get('dimensions').get('height')) if ele.get('dimensions') and ele.get(
                            ...
                            if not product and not product_without_woo_id:
                                ...
                                if ele.get('attributes'):
                                    ...
                                    for rec in ele.get('attributes'):
                                        ...
                                        pro_val = []
                                        if rec.get('option_ids'):
                                            for value in rec.get('option_ids'):
                                                existing_attr_value = self.env['product.attribute.value'].sudo().search(
                                                    [('name', '=', value),('attribute_id','=',product_attr.id)], limit=1)
                                                dict_value = {}
                                                dict_value['is_exported'] = True
